...
titles <- data.frame(titles = c("3+3", "CRM", "BOIN"), row.names = c("tpt", "crm", "boin"))

create_ui_server <- function(design) {
  questions <- data[data$design == design, ]
  ...
  ui <- shiny::fluidPage(
    ...
    shiny::titlePanel(paste(titles[design, ], "Design Parameters", sep = " ")),
    ...
    shiny::mainPanel(
      ...
      shiny::fluidRow(shiny::actionButton("get_rating",
                                  label = "Simulate!",
                                  class = "btn-block"))
    )
  )
...
# Testting
app<- create_ui_server("tpt")
shiny::shinyApp(app$ui, app$server)

app<- create_ui_server("crm")
shiny::shinyApp(app$ui, app$server)
