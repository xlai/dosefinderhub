...
con_server <- function(id) {
    moduleServer(id, function(input, output, session) {
        observeEvent(input$finalize_button, {
            showModal(modalDialog(
                title = "Design Finalized",
                "Your trial design has been finalized. You can now proceed with the implementation.",
                easyClose = TRUE,
                footer = NULL
            ))
            updateTabsetPanel(session$ns("nav"), selected = "Intro")
        })
    })
}